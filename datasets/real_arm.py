...
class RealArmDataset(BaseDataset):
    """Dataset for real robot arm demonstrations stored in zarr format."""
    quat_format = 'xyzw'
    camera_inds = None
    camera_inds2d = None
    train_copies = 10
    ...
    def __init__(
        ...
        # Load instructions first to get task list
        with open(instructions) as f:
            instr = json.load(f)
        self.tasks = list(instr.keys())
        self.cameras = None  # not used for filtering, zarr already has correct cams
